...
public class PlayerControlPhone : MonoBehaviour {
	...
	int lane = 1;
	bool canJump = false, alive = true, dl = false, isOnTopOfObstacle = false, dashing = false;
	...
	void CastSpell()
	{
		...
		if (stateInfo.fullPathHash == runStateHash)
		{
			...
			if (GameManager.SpellEquiped == Const_Script.Fireball && GameManager.Mana >= 25)
			{
			...
			else if (GameManager.SpellEquiped == Const_Script.IceCube && GameManager.Mana >= 20)
			{
				GameManager.ReduceMana();
			...
			else if (GameManager.SpellEquiped == Const_Script.ThunderCone && GameManager.Mana >= 15)
			{
				GameManager.ReduceMana();
					...
					if (distance > 75)
					{
						...
						else if (swipedSideways == false && deltaY > 0)	//swiped Down
						{
							if (canJump == true)
							{
								...
								Instantiate(sliceWave, magicHandPos.position + new Vector3(-1.35f, 2, 0), sliceWave.transform.rotation);
							}
						}
